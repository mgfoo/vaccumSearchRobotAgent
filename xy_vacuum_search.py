...
class VacuumPlanning(Problem):
    ...
    def path_cost(self, curNode, state1, action, state2):
        ...
        if (self.turnCostOn == True):
            cost += (0.5*self.computeTurnCost(curNode.action, action))
        return cost
        

    def computeTurnCost(self, action1, action):   
        costOfState = 0
        if action1 == 'UP' and (action == 'LEFT' or action == 'RIGHT'):
            costOfState += 1
        elif action1 == 'UP' and (action == 'DOWN'):
            costOfState += 2
        elif action1 == 'DOWN' and (action == 'LEFT' or action == 'RIGHT'):
            costOfState += 1
        elif action1 == 'DOWN' and action == 'UP':
            costOfState += 2
        elif action1 == 'RIGHT' and (action == 'DOWN' or action == 'UP'):
            costOfState += 1
        elif action1 == 'LEFT' and (action == 'DOWN' or action == 'UP'):
            costOfState += 1
        elif action1 == 'LEFT' and (action == 'RIGHT'):
            costOfState += 2
        elif action1 == 'RIGHT' and (action == 'LEFT'):
            costOfState += 2
        elif action1 == None and (action == None):
            costOfState += 0
            #print("Warning: computeTurnCost: not covered: ({0},{1})".format(action1,action))
        elif action1==action:
            costOfState += 0
            #print("Warning: computeTurnCost: not covered: ({0},{1})".format(action1,action))
        return costOfState
